    ...
    <div class="card-print glass border border-white/10 rounded-3xl overflow-hidden max-w-xl w-full shadow-2xl">
        ...
        <div class="relative px-8 pt-10 pb-6 text-center"
            ...
            @if($adminUser?->location)
                <p class="text-slate-500 text-xs mt-0.5">{{ $adminUser->location }}</p>
            @endif
        </div>
        ...
        <div class="px-8 py-6 space-y-5">
            ...
            <div class="space-y-3">
                ...
                @if($adminUser?->phone)
                <a href="https://wa.me/{{ preg_replace('/\D/', '', $adminUser->phone) }}" target="_blank" rel="noopener"
                   class="flex items-center gap-3 text-sm text-slate-300 hover:text-green-400 transition-colors group">
                    ...
                    <span class="font-mono text-xs">{{ $adminUser->phone }}</span>
                </a>
                @endif

                @if($adminUser?->github_url)
                <a href="{{ $adminUser->github_url }}" target="_blank" rel="noopener"
                   class="flex items-center gap-3 text-sm text-slate-300 hover:text-accent-primary transition-colors group">
                    <div class="w-9 h-9 rounded-xl glass border border-white/10 flex items-center justify-center shrink-0 group-hover:border-accent-primary/30 transition-colors">
                    ...
                    <span class="font-mono text-xs">{{ ltrim(parse_url($adminUser->github_url, PHP_URL_PATH), '/') }}</span>
                </a>
                @endif
            </div>
        ...
        <div class="px-8 py-4 border-t border-white/5 flex items-center justify-between text-[10px] text-slate-600 font-mono">
            <span>&lt;PE/&gt;</span>
            @if($adminUser?->availability_text)
                <span>{{ $adminUser->availability_text }}</span>
            @elseif($adminUser?->is_available)
                <span>Disponible — Remote & {{ $adminUser?->location ?? 'Libreville' }}</span>
            @else
                <span>Non disponible actuellement</span>
            @endif
        </div>
    </div>
