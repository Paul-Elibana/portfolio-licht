    ...
    <section id="hero" class="relative min-h-screen flex items-center pt-24 pb-16 overflow-hidden">
        ...
        @if(isset($assets['hero']) && $assets['hero']->first())
            <div class="absolute inset-0 pointer-events-none" style="z-index:0">
                <img src="{{ $assets['hero']->first()->url }}"
                     alt="Bannière de fond du portfolio"
                     loading="lazy"
                    ...
                    <x-glass-card class="flex flex-col gap-4 p-0 overflow-hidden">
                        <a href="{{ asset('storage/' . $cert->document_path) }}" target="_blank" rel="noopener"
                           class="block w-full overflow-hidden bg-slate-900 relative" style="height:176px;">
                            @if($certIsPdf)
                                <div class="absolute inset-0 flex items-center justify-center bg-slate-900 pdf-skeleton">
                                ...
                                <canvas class="pdf-thumb w-full h-full relative"
    ...
    s.onload = () => {
        ...
        canvases.forEach(canvas => {
            pdfjsLib.getDocument(canvas.dataset.pdfSrc).promise
                ...
                .then(() => canvas.parentElement.querySelector('.pdf-skeleton')?.remove())
                .catch(() => canvas.parentElement.querySelector('.pdf-skeleton')?.remove());
        });
    };
